...
def spectrogram2wav_for_ppg_cbhg(spec_pred, gl_power = 1., gl_iter = hparams['iterations']):
    power_spec = db2power(log_power_denormalize(spec_pred))
    mag_spec = power_spec ** 0.5
    y = griffin_lim(mag_spec ** gl_power, gl_iter)
    y = deemphasis(y)
    return y
